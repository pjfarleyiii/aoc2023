      ...
      * SORTING VARIABLES
       01  TEMPITEM.
           05  TMPPARTNO         PIC  9(4).
           05  TMPPARTSYM.
               10  TMPSYMBVAL    PIC  X.
               10  TMPSYMBLINE   PIC  9(4).
               10  TMPSYMBLOC    PIC  9(4).
       ...
       01  GAME-VARIABLES.
           ...
           05  GEARRATIO         PIC  9(8) VALUE ZEROES.
           05  GEARCNT           PIC  9(8) VALUE ZEROES.
       ...
       01  VALID-PARTS-TABLE.
           ...
           05  VAL-PART-AREA.
               10  VALPART-TABLE OCCURS 1 TO 2560 TIMES
                   ...
                   15  VALPARTSYM.
                       20  VALSYMBVAL   PIC  X.
                       20  VALSYMBLINE  PIC  9(4).
                       20  VALSYMBLOC   PIC  9(4).
                       ...
                       IF MAPLINE (SS1)
                          ...
                          NOT = "."
                               ...
                               MOVE MAPLINE (SS1)
                                            (PARTLOC (PARTMAX) - 1 : 1)
                                 TO SYMBVAL (PARTMAX)
                               COMPUTE SYMBLINE (PARTMAX) = SS1
                               COMPUTE SYMBLOC (PARTMAX) =
                                       PARTLOC (PARTMAX) - 1
                       END-IF
                       IF NOT PARTVALID (PARTMAX) AND
                          ...
                          NOT = "."
                               ...
                               COMPUTE SYMBLOC (PARTMAX) =
                                   ...
                                   PARTLEN (PARTMAX)
                       END-IF
      D                DISPLAY "PARTNO[" PARTMAX "] @ "
      D                    PARTLOC (PARTMAX) " = '" PARTNO (PARTMAX)
      D                    "' FOR LEN " PARTLEN (PARTMAX)
      D                    " VALID='" PARTVALSW (PARTMAX) "'"
      D                    " SYMBOL=" PARTSYM   (PARTMAX)
                   END-IF
               END-PERFORM
               IF PARTMAX = 0
      ...
      *            PREVIOUS OR NEXT LINE
      ...
      D                       "SRCHAREA(-1)='" SRCHAREA "'"
                           ...
                           SEARCH MAPSRCH VARYING SS3
                               ...
                               WHEN MAPSRCH (SS3) NOT = "." AND
                                   ...
                                   MOVE MAPSRCH (SS3) TO SYMBVAL (SS2)
                                   COMPUTE SYMBLINE (SS2) = SS1 - 1
                                   COMPUTE SYMBLOC  (SS2) =
                                       PARTLOC (SS2) - 2 + SS3
                           END-SEARCH
                       END-IF
      ...
      D                       "SRCHAREA(+1)='" SRCHAREA "'"
                       ...
                       IF PARTVALID (SS2)
                           ...
                           MOVE PARTSYM (SS2) TO VALPARTSYM (VALPARTMAX)
      D                    DISPLAY "PARTNO[" SS2 "] @ "
      D                        PARTLOC (SS2) " = '" PARTNO (SS2)
      D                        "' FOR LEN " PARTLEN (SS2)
      D                        " VALID='" PARTVALSW (SS2) "'"
      D                        " SYMBOL=" PARTSYM   (SS2)
                       END-IF
                   END-PERFORM
      *            PERFORM SORT-VALID-BY-PARTNO
      D            DISPLAY " "
      ...
      D                "," VALPARTSYM (SS1)
           ...
           END-PERFORM
           PERFORM SORT-VALID-BY-PARTSYM
           PERFORM VARYING SS1 FROM +1 BY +1
               UNTIL SS1 > VALPARTMAX
      *           OR VALSYMBVAL (SS1) > "*"
      D        DISPLAY "GEAR  PART[" SS1 "]=" VALPARTNO (SS1)
      D            "," VALPARTSYM (SS1)
               IF VALSYMBVAL (SS1) = "*"
                   IF VALPARTSYM (SS1) = VALPARTSYM (SS1 + 1)
                       ADD 1 TO GEARCNT
                       COMPUTE GEARRATIO =
                           VALPARTNO (SS1) * VALPARTNO (SS1 + 1)
      D                DISPLAY "GEAR RATIO[" SS1 "]=" GEARRATIO
      D                    "= " VALPARTNO (SS1)
      D                    " * " VALPARTNO (SS1 + 1)
                       ADD GEARRATIO TO PART2-VALUE
                   END-IF
               END-IF
           END-PERFORM
           DISPLAY "PART1=" PART1-VALUE
           DISPLAY "PART2=" PART2-VALUE ",GEARCNT=" GEARCNT
           CONTINUE.
      ...
      D    END-IF
           ...
           PERFORM WITH TEST AFTER UNTIL HASNOTCHANGED
              ...
              PERFORM VARYING ITEMINDEX FROM 1 BY 1
                 ...
                 IF VALPARTNO (ITEMINDEX) > VALPARTNO (ITEMINDEX + 1)
                    MOVE VALPART-TABLE (ITEMINDEX) TO TEMPITEM
                    MOVE VALPART-TABLE (ITEMINDEX + 1)
                      TO VALPART-TABLE (ITEMINDEX)
                    MOVE TEMPITEM TO VALPART-TABLE (ITEMINDEX + 1)
       ...
       SORT-VALID-BY-PARTSYM.
           ...
           PERFORM WITH TEST AFTER UNTIL HASNOTCHANGED
              ...
              PERFORM VARYING ITEMINDEX FROM 1 BY 1
                 UNTIL ITEMINDEX > ITEMCOUNT
                 IF VALPARTSYM (ITEMINDEX) > VALPARTSYM (ITEMINDEX + 1)
                    MOVE VALPART-TABLE (ITEMINDEX) TO TEMPITEM
                    MOVE VALPART-TABLE (ITEMINDEX + 1)
                      TO VALPART-TABLE (ITEMINDEX)
                    MOVE TEMPITEM TO VALPART-TABLE (ITEMINDEX + 1)
